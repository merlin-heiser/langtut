...
try {
  ...
  if (await exists(path.join(root, "activities.yaml"))) { const doc = YAML.parse(await readFile(path.join(root, "activities.yaml"), "utf8")); const activityIds = new Set(); for (const activity of doc.activities ?? []) { if (!activity.id || activityIds.has(activity.id)) throw new Error(`Duplicate activity: ${activity.id}`); activityIds.add(activity.id); const roles = new Set((activity.roles ?? []).map((role) => role.id)); if (!activity.turnOrder?.length || activity.turnOrder.length !== roles.size || activity.turnOrder.some((id) => !roles.has(id))) throw new Error(`${activity.id}: invalid turn order`); for (const role of activity.roles ?? []) { if (!new Set(["learner", "llm", "fixed"]).has(role.controller)) throw new Error(`${activity.id}: invalid controller ${role.controller}`); if (role.controller === "llm" && role.prompt && !knownPrompts.has(role.prompt)) throw new Error(`${activity.id}: unknown prompt ${role.prompt}`); if (role.controller === "fixed" && !role.message) throw new Error(`${activity.id}: fixed role needs message`); } } }
