...
function Get-AdbDevices {
  @(adb devices -l | Select-Object -Skip 1 | Where-Object { $_ -match '\tdevice\s' } | ForEach-Object {
    $parts = $_ -split '\s+'
    [pscustomobject]@{
      Serial = $parts[0]
...
try {
  & powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/android-local-debug.ps1
