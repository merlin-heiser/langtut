...
return {
  ...
  startPlacement: () => post("/placement-sessions"), answerPlacement: (id, itemId, answer) => post(`/placement-sessions/${encodeURIComponent(id)}/answers`, { itemId, answer }), overridePlacement: (id, moduleId) => post(`/placement-sessions/${encodeURIComponent(id)}/override`, { moduleId }), prepareModule: (id) => post(`/modules/${encodeURIComponent(id)}/prepare`), createSessionPlan: () => post("/session-plans"), startSession: (input) => post("/sessions", input), activityTurn: (id, message) => post(`/sessions/${encodeURIComponent(id)}/activity-turns`, { message }), recordMilestone: (moduleId, milestoneId) => post(`/modules/${encodeURIComponent(moduleId)}/milestones/${encodeURIComponent(milestoneId)}/attempt`),
