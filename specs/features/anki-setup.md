...
Neue Langtut-Karten werden standardmäßig ausgesetzt. Automatisch freigegeben
werden ausschließlich Vokabelkarten eines Moduls mit Domain-Status `learning`;
Chunk- und Regelkarten bleiben ausgesetzt, bis eine eigene Systemregel sie
freigibt. Karten aus `locked`, `available`, `preparing` und `credited`-Modulen
bleiben ausgesetzt. Die Regel wird bei jedem Curriculum-Abgleich auch auf bereits
importierte Langtut-Karten angewendet. Fremde Karten und Modelle werden nicht
verändert.
